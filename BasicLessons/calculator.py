...
# get the user input and place it in the textfield(input box)
i = 0
def get_variables(num):
    global i

    #if i <= 8:
    display.insert(i, num)  # puts the number the user clicks into the display
...
def calculate():
    ...
    try:
        ...
        result = eval(a) # will evaluate the entire expression
        clear_all()
...
display = Entry(root)
display.grid(row=1, columnspan=6, sticky=E+W)   # sticky makes it run west to east
...
Button(root, text="x!", command=lambda: get_operation(factorial(display.get(-1)))).grid(row=3, column=5)    # no work :(
